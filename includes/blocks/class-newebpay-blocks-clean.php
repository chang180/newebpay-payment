...
class Newebpay_Blocks {
    ...
    public function register_rest_routes() {
        ...
        register_rest_route( 'newebpay/v1', '/payment-status/(?P<order_id>\d+)', array(
            'methods' => 'GET',
            'callback' => array( $this, 'rest_get_payment_status' ),
            'permission_callback' => array( $this, 'rest_order_permission_check' ),
            'args' => array(
                'order_id' => array(
                    'required' => true,
        ...
        register_rest_route( 'newebpay/v1', '/block-settings', array(
            ...
            'permission_callback' => array( $this, 'rest_editor_permission_check' )
        ) );
    }
    ...
    public function rest_get_payment_methods( $request ) {
        ...
        return new WP_REST_Response( array(
            ...
            'data' => array(
                ...
                'all' => $all_methods,
                'enabled_count' => count( $available_methods ),
                'version' => '1.0.10'
            )
        ), 200 );
    }
     ...
     * REST API: 取得付款狀態
     */
    public function rest_get_payment_status( $request ) {
        $order_id = $request['order_id'];
        
        if ( ! function_exists( 'wc_get_order' ) ) {
            return new WP_Error( 'woocommerce_not_found', 'WooCommerce is not active', array( 'status' => 404 ) );
        }
    ...
    public function rest_get_block_settings( $request ) {
        ...
        return new WP_REST_Response( array(
            'success' => true,
            'data' => array(
                'newebpay_enabled' => ! empty( $nwp_settings ),
     ...
     * REST API 編輯器權限檢查
     */
    public function rest_editor_permission_check( $request ) {
     ...
     * REST API 訂單權限檢查
     */
    public function rest_order_permission_check( $request ) {
        $order_id = $request['order_id'];
